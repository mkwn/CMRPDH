...
\section{Perfect Matching in Hypergraphs}
\begin{defn}
...
bound over all choices of $W$. Our approach is to use Szemeredi's
regularity lemma on $G$ and to show that the random matching a.a.s.
distributes evenly over the clusters. Then, $\varepsilon$-regularity
...
constant $K$ depending only on $\alpha$ such that we can write $A=V_{0}^{1}\sqcup\dots\sqcup V_{k}^{1}$
and $B=V_{0}^{2}\sqcup\dots\sqcup V_{k}^{2}$ with $k\le K$, in such
a way that the following conditions are satisfied. The ``exceptional''
clusters $V_{0}^{1}$ and $V_{0}^{2}$ both have fewer than $\varepsilon n$
vertices, and the non-exceptional clusters in $A$ and $B$ have equal
size: $\left|V_{1}^{1}\right|=\dots=\left|V_{k}^{\ell}\right|=an$
...
Now, if $V_{j}^{2}\in N_{\mathfrak{G}}\left(\mathfrak{H}\right)$
then by $\varepsilon$-regularity there are edges in $G'$ from $W$
to at least $\left(1-\varepsilon\right)bn$ vertices of $V_{j}^{2}$.
...
with $\left|S\right|=\alpha'k$ and let $S$ be the vertices in the
clusters of $\mathfrak{S}$. By the same $\varepsilon$-regularity
argument, $\left|N_{G'}\left(W\right)\cap S\right|\ge\left(1-\varepsilon\right)\alpha'kbn\ge\left(1-\varepsilon\right)^{2}\alpha'n$.
There is $V_{j}^{2}$ outside $N_{\mathfrak{G}}\left(\mathfrak{H}\right)$
which has $\alpha'k$ neighbours outside $\mathfrak{H}$ in $\mathfrak{G}$,
so $\left|\mathfrak{H}\right|\le\left(1-\alpha'\right)k$. Each $V_{i}^{1}$
has at least $\left(a-\xi\left(\alpha\right)\right)n$ neighbours
in $M$, at most $\alpha'k\left(ab+\xi\left(\alpha\right)\right)n$
of which are in $S$. So, 
\begin{align*}
...
Our chosen matching $M_{0}$ can be used to give a uniformly random
partition of $B_{0}$ into pairs of vertices: for every $\left\{ a,b,b'\right\} \in M_{0}$,
pair up $b$ and $b'$, and partition the remaining $2\varepsilon n/3$
elements of $B_{0}$ into pairs uniformly at random. Let $B$ be the
resulting set of pairs of elements of $B_{0}$. Conditioning on $A$
and $B$, $M_{0}$ is distributed uniformly at random among all matchings
consisting of $\left(1-\varepsilon\right)n/3$ hyperedges between
a pair of $B$ and a vertex of $A$.

We consider only the hyperedges in $H\cup R$ between a vertex of
$A$ and a pair of $B$. Each of these hyperedges can be viewed as
an edge in the bipartite graph with partitions $A$ and $B$. The
theorem will follow from Lemma \ref{lem:bipartite-plus-big-matching-perfect}
if we can show that with a uniformly random choice of $A$ and $B$,
$H$ induces a bipartite graph $G$ with linear minimum degree.

So, let $a_{1},\dots,a_{n/3},b_{n/3+1},\dots,b_{n}$ be a uniformly
random ordering of $V\left(H\right)$. Let $A$ contain the first
$n/3$ vertices, let $B_{0}=V\left(H\right)\backslash A$ and let
$B$ be the set of pairs $\left\{ b_{2i-1},b_{2i}\right\} $. Let
$\Gamma\left(v,w\right)$ (respectively $\Gamma\left(v\right)$) be
the set of all vertices $u$ (respectively, all pairs of vertices
$\left\{ w,u\right\} $) such that $\left\{ v,w,u\right\} $ is a
hyperedge of $H$. We will first prove that each $\Gamma\left(v,w\right)$
is a.a.s. almost evenly distributed between $A$ and $B_{0}$. So,
let $\varepsilon>0$ be a small constant (depending on $\alpha$),
and fix distinct vertices $v,w$.
...
By a union bound, a.a.s. every pair of vertices $v,w$ has at least
$\left(\alpha/3-\varepsilon\right)n$ neighbours in $A$ and at least
$\left(2\alpha/3-\varepsilon\right)n$ neighbours in $B_{0}$.

The degree of each $\left\{ b_{2i-1},b_{2i}\right\} \in B$ in $G$
is $\left|N\left(b_{2i-1},b_{2i}\right)\cap A\right|$, which we have
just shown is uniformly$\Omega\left(n\right)$. It remains to show
that the degree of each $a\in A$ in $G$ is $\Omega\left(n\right)$.
So, fix $a\in A$. For each $b$, $\left|\Gamma\left(a,b\right)\cap B_{0}\right|\ge\left(2\alpha/3-\varepsilon\right)n$,
...
\]
Now, condition on $b_{1},\dots,b_{2i}$. The probability that $\left\{ a,b_{2i+1},b_{2i+2}\right\} $
is in $H$ is (for large $n$) at least 
\[
\left|\Gamma\left(a\right)\right|-2i\left|B_{0}\right|/{\left|B_{0}\right| \choose 2}\ge\left(\alpha-2\varepsilon-6i/n\right)n,
\]
which is at least $\alpha/2$ for $i\le\left(\alpha/12-\varepsilon/3\right)n=\Omega\left(n\right)$.
For such $i$, let $X_{i}$ be independently distributed as $\Ber\left(\alpha/2\right)$.
By a Chernoff bound, $\sum_{i=1}^{\Omega\left(n\right)}X_{i}$ (and
therefore $\left|N_{G}\left(a\right)\right|$) is $\Omega\left(n\right)$
with probability $e^{-\Omega\left(n\right)}$. By a union bound, it
follows that a.a.s. $\delta_{G}\left(a\right)=\Omega\left(n\right)$.
\end{proof}
