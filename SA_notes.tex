...
\section{Hamiltonicity in hypergraphs}
\begin{defn}
The 2-degree of a pair of vertices $v,w$ is the number of hyperedges
containing both $v$ and $w$.
\end{defn}

\begin{defn}
Let $\HH\left(n,m\right)$ be the random 3-uniform hypergraph on the
vertex set $\left[n\right]$ chosen uniformly from all 3-uniform hypergraphs
with $m$ hyperedges.\end{defn}
\begin{thm}
\label{thm:hypergraph-perfect-matching}There is $\beta\left(\alpha\right)$
such that if $H$ is a 3-uniform hypergraph on $\left[n\right]$ with
minimal 2-degree at least $\alpha n$ and $R\in\HH\left(n,\beta\left(\alpha\right)n\right)$,
then $H\cup R$ a.a.s. has a perfect matching.
\end{thm}
This theorem will follow from a lemma.
\begin{lem}
\label{lem:bipartite-plus-big-matching-perfect}There is $\xi\left(\alpha\right)$
...
\begin{proof}
We use Hall's marriage theorem: we need to show that $\left|N_{G\cup M}\left(W\right)\right|\ge\left|W\right|$
...
It is straightfoward to prove that for any individual such $W$, a.a.s.
$\left|N_{G\cup M}\left(W\right)\right|\ge\left|W\right|$. However
the probability of failure is not small enough for a union bound over
all choices of $W$. Our approach is to use Szemeredi's regularity
lemma on $G$ and to show that the random matching a.a.s. distributes
evenly over the clusters. Then, $\varepsilon$-regularity and the
even distribution of the added edges are enough to ensure that $\left|N_{G\cup M}\left(W\right)\right|\ge\left|W\right|$
for each $W$.
...
be a small constant that will be determined later ($\varepsilon<\alpha/8$).
There is a large constant $K$ depending only on $\alpha$ such that
we can write $A=V_{0}^{1}\sqcup\dots\sqcup V_{k}^{1}$ and $B=V_{0}^{2}\sqcup\dots\sqcup V_{k}^{2}$
with $k\le K$, in such a way that the following conditions are satisfied.
The ``exceptional'' sets $V_{0}^{1}$ and $V_{0}^{2}$ both have
fewer than $\varepsilon n$ vertices, and the non-exceptional clusters
in $A$ and $B$ have equal size: $\left|V_{1}^{1}\right|=\dots=\left|V_{k}^{\ell}\right|=an$
...
Fix some $V_{i}^{1}$ and $V_{j}^{2}$. The vertices of each $V_{i}^{1}$
are matched by $M$ to at most $an$ vertices; conditioning on their
number, they are uniformly randomly chosen from $B$. Let $X_{b}$
be the indicator random variable for the event that the vertex $b\in B$
is matched with a vertex in $V_{i}^{1}$. The random variables $X_{b}$
are said to be \emph{negatively associated} (see \cite[Application 3.1(c)]{JDP83}).
This means in particular that we can apply Chernoff bounds to $\sum_{b\in V_{j}^{2}}X_{b}$
...
of $M$ between every pair $V_{i}^{1}$, $V_{j}^{2}$.

Now, in $G'$ each $V_{i}^{\ell}$ has at least $\left(\alpha'+\varepsilon\right)n\left|V_{i}^{\ell}\right|$
edges to other clusters. There are at most $\varepsilon n\left|V_{0}^{\ell}\right|$
...
Consider any $W\subset A$ with $\alpha n\le\left|W\right|\le\ceil{n/2}$.
For each $i$ let $\pi_{i}=\left|V_{i}^{1}\cap W\right|/\left(an\right)$,
and let $Q$ be the set of clusters $V_{i}^{1}$ with $\pi_{i}\ge\varepsilon$.
Now, if $V_{j}^{2}\in N_{\mathfrak{G}}\left(Q\right)$ then in $G'$,
by $\varepsilon$-regularity there are edges from $W$ to at least
$\left(1-\varepsilon\right)bn$ vertices of $V_{j}^{2}$.

It follows that if $\left|N_{\mathfrak{G}}\left(Q\right)\right|=k$
(that is, every $V_{i}^{\ell}$ is a neighbour of $Q$) then $\left|N_{G\cup M}\left(W\right)\right|\ge\left|N_{G'}\left(W\right)\right|\ge\left(1-\varepsilon\right)\left(n-\left|V_{0}^{2}\right|\right)\ge\left|W\right|$
and we are done. Otherwise, choose $S\subseteq N\left(Q\right)$ with
$\left|S\right|=\beta k$, so that $W$ has $\left(1-\varepsilon\right)\beta kbn\ge\left(1-\varepsilon\right)^{2}\beta n$
neighbours among the clusters in $S$. There is $V_{j}^{2}$ outside
$N_{\mathfrak{G}}\left(Q\right)$ which has $\beta k$ neighbours
outside $Q$ in $\mathfrak{G}$, so $\left|Q\right|\le\left(1-\beta\right)k$.
Each $V_{i}^{1}$ has at least $\left(a-\xi\left(\alpha\right)\right)n$
neighbours in $M$, at most $\beta k\left(ab+\xi\left(\alpha\right)\right)n$
of which are in $S$. So, $W$ has at least 
\[
\sum_{V_{i}^{1}\in Q}\pi_{i}an-\left(\beta k\left(ab+\xi\left(\alpha\right)\right)+\xi\left(\alpha\right)\right)n\left|Q\right|-\xi\left(\alpha\right)\ge\left|W\right|-\varepsilon n-\beta\left(1-\beta\right)\left(\left(1-\varepsilon\right)^{2}+k^{2}\xi\left(\alpha\right)\right)n-k\xi\left(\alpha\right)n
\]
neighbours outside $S$. Combining with the $\left(1-\varepsilon\right)^{2}\beta n$
neighbours of $W$ in $G'$ inside $S$, we have
\[
\left|N_{G'\cup M}\left(W\right)\right|\ge\left|W\right|+\left(\beta\left(1-\varepsilon\right)^{2}-\varepsilon-\beta\left(1-\beta\right)\left(\left(1-\varepsilon\right)^{2}+k^{2}\xi\left(\alpha\right)\right)-k\xi\left(\alpha\right)\right)n.
\]
...
[Proof of Theorem \ref{thm:hypergraph-perfect-matching}]The idea
is to reduce this theorem to Lemma \ref{lem:bipartite-plus-big-matching-perfect}:
roughly speaking, $H$ induces a bipartite graph, and $R$ provides
a uniformly random large matching. First, uniformly randomly partition
$V\left(H\right)$ into a set $A$ of $n/3$ vertices and a set $B_{0}$
of $2n/3$ vertices. 

We can construct a random hypergraph distributed as $\HH\left(n,\beta\left(\alpha\right)n\right)$
by repeatedly choosing random hyperedges with repetition, and stopping
when we have seen $\beta\left(\alpha\right)n$ distinct hyperedges.
We will show that for any $\varepsilon$ there is $\beta$ such that
a.a.s. the first $\beta\left(\alpha\right)n$ random edges will contain
a matching $M_{0}$ of $\left(1-\varepsilon\right)n/3$ hyperedges,
each comprising a vertex of $A$ and two vertices of $B_{0}$. This
matching can be chosen greedily: each new random hyperedge is immediately
added to $M_{0}$ if it has one vertex in $A$ and two vertices in
$B_{0}$, and does not conflict with any hyperedge added so far. If
our matching-in-progress has $i$ hyperedges, the probability that
the next random edge will be added is 
\[
p_{i}=\left(n/3-i\right)\left.{2n/3-2i \choose 2}\right/{n \choose 3}=\Omega\left(\left(\frac{n-i}{n}\right)^{3}\right).
\]
The number of random edges required to build a matching of size $\left(1-\varepsilon\right)n/3$
...
Our chosen matching $M_{0}$ couples with a uniformly random partition
of $B_{0}$ into pairs: for every $\left\{ a,b,b'\right\} \in M$,
pair up $b$ and $b'$, and partition the remaining $2\varepsilon n/3$
elements of $B_{0}$ into pairs uniformly at random. Conditioning
on the set of pairs $B$, $M_{0}$ is distributed uniformly at random
among all matchings consisting of $\left(1-\varepsilon\right)n/3$
hyperedges between a pair of $B$ and a vertex of $A$.

In order to apply Lemma \ref{lem:bipartite-plus-big-matching-perfect},
we just need to show that a uniformly random choice of $A,B$ induces
a bipartite graph with linear minimum degree.

Let $a_{1},\dots,a_{n/3},b_{n/3+1},\dots,b_{n}$ be a uniformly random
ordering of $V\left(H\right)$. Let $A$ contain the first $n/3$
vertices, let $B_{0}=V\left(H\right)\backslash A$ and let $B$ be
the set of pairs $\left\{ b_{2i-1},b_{2i}\right\} $. Let $\Gamma\left(v\right)$
be the set of all pairs of vertices $\left\{ w,u\right\} $ and let
$\Gamma\left(v,w\right)$ be the set of all vertices $u$ such that
$\left\{ v,w,u\right\} $ is a hyperedge of $H$. We will first prove
that each $\Gamma\left(v,w\right)$ is a.a.s. almost evenly distributed
between $A$ and $B_{0}$. So, let $\varepsilon>0$ be some small
constant to be determined later, and fix distinct vertices $v,w$.

There are at least $\alpha n$ vertices $u$ such that $\left\{ u,v,w\right\} $
is a hyperedge in $H$. Let $X_{i}^{v,w}$ be the indicator for the
event that the $i$th vertex of our random ordering is in $\Gamma\left(v,w\right)$.
By \cite[Application 3.1(c)]{JDP83}, $\left\{ X_{i}^{v,w}\right\} _{i=1}^{n}$
is a set of negatively associated random variables. Let $X_{A}=\sum_{i=1}^{n/3}X_{i}^{v,w}=\left|A\cap\Gamma\left(v,w\right)\right|$
and $X_{B}=\sum_{i=n/3+1}^{n}X_{i}^{v,w}=\left|B_{0}\cap\Gamma\left(v,w\right)\right|$,
...
\]
Similarly, $\Pr\left(\left|B_{0}\cap\Gamma\left(v,w\right)\right|<\left(2\alpha/3-\varepsilon\right)n\right)\le e^{-\Omega\left(n\right)}$,
so a.a.s. every pair of vertices $v,w$ has at least $\left(\alpha/3-\varepsilon\right)n$
neighbours in $A$ and at least $\left(2\alpha/3-\varepsilon\right)n$
neighbours in $B_{0}$.

The degree of each $\left\{ b_{2i-1},b_{2i}\right\} \in B$ in $G$
is $\left|N\left(b_{2i-1},b_{2i}\right)\right|$, which we have just
shown is uniformly$\Omega\left(n\right)$. It remains to show that
the degree of each $a\in A$ is $\Omega\left(n\right)$. So, fix $a\in A$.
For each $b$, $\left|\Gamma\left(a,b\right)\cap B_{0}\right|\ge\left(2\alpha/3-\varepsilon\right)n$,
so $\Gamma\left(a\right)\ge\left(2\alpha/3-\varepsilon\right)\left|B_{0}\right|n/2$.
The probability that $\left\{ a,b_{1},b_{2}\right\} $ is a hyperedge
in $H$ is therefore (for large $n$) at least
\[
\left|\Gamma\left(a\right)\right|/{\left|B_{0}\right| \choose 2}\ge\alpha-2\varepsilon.
\]
Now, condition on $b_{1},\dots,b_{2i}$. There are at most $2i\left|B_{0}\right|$
hyperedges containing $a$ and some conditioned $b_{j}$, so the probability
that $\left\{ a,b_{2i+1},b_{2i+2}\right\} $ is in $H$ is at least
$\alpha-2\varepsilon-6i/n$, which is at least $\alpha/2$ for $i\le\left(\alpha/12-\varepsilon/3\right)n$.
For such $i$, let $X_{i}$ be independently distributed as $\Ber\left(\alpha/2\right)$.
By a Chernoff bound, $\sum_{i=1}^{\left(\alpha/12-\varepsilon/3\right)n}X_{i}$
(and therefore $\left|N\left(a\right)\cap B\right|$) is $\Omega\left(n\right)$
with probability $e^{-\Omega\left(n\right)}$. By a union bound, it
follows that a.a.s. $d_{G}\left(a\right)=\Omega\left(n\right)$.
\end{proof}
