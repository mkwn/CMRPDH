...
\title{Hamiltonicity in randomly perturbed digraphs and hypergraphs}
...
become Hamiltonian after a modest random perturbation. First, we show
that adding linearly many random edges to a dense hypergraph ensures
the (asymptotically almost sure) existence of a matching or a loose
...
number of random edges makes a dense digraph pancyclic. Finally, we
show that perturbing a certain (minimum-degree-dependent) number of
random edges in a tournament ensures the existence of multiple edge-disjoint
...
of Dirac \cite[Theorem~3]{Dir52}, which states that any $n$-vertex
graph with minimum degree at least $n/2$ is Hamiltonian.
...
\emph{a.a.s.}). This follows from a stronger result (\cite[Theorem~3]{Pos76})
...
tight. We can interpret \cite[Theorem~1]{BFM03} as quantifying the
``fragility'' of the few dense graphs that are not Hamiltonian,
by determining the amount of random perturbation that is necessary
to make a dense graph Hamiltonian. A comparison can be drawn to the
notion of \emph{smoothed analysis} of algorithms introduced in \cite{ST04},
which involves studying the performance of algorithms on randomly
perturbed inputs.

Our first contribution in this paper is to generalize \cite[Theorem~1]{BFM03}
to hypergraphs (and to give a corresponding result for perfect matchings,
which is nontrivial in the hypergraph setting). Unfortunately, there
is no single most natural notion of a cycle or of minimum degree in
hypergraphs. A $k$-uniform \emph{loose} cycle is a $k$-uniform hypergraph
with a cyclic ordering on its vertices such that every edge consists
of $k$ consecutive vertices and every pair of consecutive edges intersects
in exactly one vertex. The degree of a set of vertices is the number
of edges that include that set, and the \emph{minimum $\left(k-1\right)$-degree
}$\delta_{k-1}$ is the minimum degree among sets of $k-1$ vertices.
\begin{thm}
...
\end{enumerate}
\end{thm}
We can motivate our \ref{thm:hypergraph-theorems} in exactly the
same way as \cite[Theorem~1]{BFM03}. There is a corresponding Dirac-type
theorem for loose Hamilton cycles in dense hypergraphs (see \cite{KKMO11}),
and the threshold for both perfect matchings and loose Hamilton cycles
is $n^{k-1}\log n$ random edges (see \cite{DF11} and \cite[Corollary~2.6]{JKV08}),
so ``almost all'' dense hypergraphs have Hamilton cycles and perfect
matchings. However, for hypergraphs with large uniformity our conclusion
is much more extreme than that of \cite[Theorem~1]{BFM03}: starting
from a dense hypergraph only $O\left(n\right)$ extra edges are requred
to ensure Hamiltonicity, which is far fewer than the $\Omega\left(n^{k-1}\log n\right)$
...
ineffective in the hypergraph setting, so our proof is very different
to the proofs found in \cite{BFM03}. In particular, we cannot easily
manipulate paths for P\'osa-type arguments, and we do not have an
analogue of Hall's marriage theorem allowing us to deduce the existence
of a perfect matching from an expansion property. Our proof involves
reducing the theorem to the existence of a perfect matching in a certain
randomly perturbed bipartite graph. The ``reason'' for a perfect
matching in this perturbed graph seems to be quite different depending
on the structure of the initial bipartite graph, so we make critical
use of the structural description given by Szemer\'edi's regularity
lemma.
...
of \cite[Theorem~1]{BFM03}, the corresponding result for digraphs
(\cite[Theorem~3]{BFM03}), and the analogous results for pancyclicity.
We will prove \ref{lem:pseudorandom-pancyclic} and its corollaries
in \ref{sec:digraphs}.
...
edges to each other cluster. So, $d_{C}\left(V_{i}^{\ell}\right)\ge\left(\left(\alpha'+\varepsilon\right)n\left|V_{i}^{\ell}\right|-\varepsilon n\left|V_{i}^{\ell}\right|\right)/\left|V_{i}^{\ell}\right|^{2}\ge\alpha'k$
...
$M$ to at most $an$ vertices; conditioning on their number, these
are uniformly randomly chosen from $B$. Let $X_{b}$ be the indicator
random variable for the event that the vertex $b\in B$ is matched
with a vertex in $V_{i}^{1}$. The random variables $X_{b}$ are said
to be \emph{negatively associated} (see \cite[Application~3.1(c)]{JDP83}).
This means in particular that we can apply Chernoff bounds to $\sum_{b\in V_{j}^{2}}X_{b}$
even though the $X_{b}$ are not independent, so with probability
$1-e^{\Omega\left(n\right)}$ there are at least $\left(ab+\xi\left(\alpha\right)\right)n$
edges of $M$ between $V_{i}^{1}$ and $V_{j}^{2}$. (We could avoid
the use of negative association with the argument we will use to prove
\ref{lem:hypergraph-bipartite-degree-transfer}). By a union bound,
a.a.s. there are at most $\left(ab+\xi\left(\alpha\right)\right)n$
edges of $M$ between every pair $V_{i}^{1}$, $V_{j}^{2}$. That
is, $M$ spreads out evenly between the clusters. We assume this holds
...
Otherwise, choose $E\subseteq N\left(D\right)$ with $\left|E\right|=\alpha'k$
and let $S$ contain the vertices in the clusters of $E$. By the
same $\varepsilon$-regularity argument, $\left|N_{G'}\left(W\right)\cap S\right|\ge\alpha'k\left(1-\varepsilon\right)bn\ge\left(1-\varepsilon\right)^{2}\alpha'n$.
...
\end{proof}
Now we describe the reduction of \ref{thm:tournament} to \ref{lem:bipartite-plus-big-matching-perfect}.
...
cycle (as provided by \ref{lem:hypergraph-almost-object}), then there
...
$B$ are uniformly random. We give one final lemma, establishing that
the density of $H$ corresponds a.a.s. to density of $G_{A,B}\left(H\right)$.
\begin{lem}
\label{lem:hypergraph-bipartite-degree-transfer}$ $

\begin{enumerate}[topsep=0px,label=(\alph*)]

\item{There is $\amat\left(\alpha\right)>0$ such that the following
holds. Let $H$ satisfy the conditions of \ref{thm:hypergraph-theorems}(a),
...
\item{There is $\aham\left(\alpha\right)>0$ such that the following
holds. Let $H$ satisfy the conditions of \ref{thm:hypergraph-theorems}(b),
let $A$ be a uniformly random set of $n$ vertices, and let $B$
...
helpful to conceptualize the randomness in a certain way. Let 
\[
...
shares at most ${n \choose k-2}$ edges with $a_{i}$, so if $j\le2\sqrt{\amat\left(\alpha\right)}n$
...
a corollary we also prove the following theorem extending \cite[Theorem~3]{BFM03}
to pancyclicity.
\begin{thm}
...
that this was always the case, which we have verified for \cite[Theorems~1 and~3]{BFM03}
(note that \ref{thm:smoothed-pancyclic} immediately implies the corresponding
...
In particular, we have only studied loose Hamiltonicity in $\left(k-1\right)$-dense
hypergraphs. The other most popular notion of a hypergraph cycle is
a \emph{tight} cycle, in which every consecutive pair of edges in
the cycle intersects in $k-1$ vertices. There are a large variety
of Dirac-type theorems for different types of minimum degree and different
types of cycles (see \cite{RR10} for a survey), which would suggest
that similar random perturbation results are possible in these settings.
In addition, it is possible that our use of Szemer\'edi's regularity
lemma could be avoided, thereby drastically improving the constants
$\chypham\left(\alpha\right)$ and $\chypmat\left(\alpha\right)$.
